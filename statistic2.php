    ...
    <table border="1px">
        <tr>
            ...
            for($j = 1; $j <=$candidate_num; $j++){
                ...
                $result = mysql_query("select * from score_backup where decision_id = '".$_GET['decision_id']."' and user_id = '".$i."' and criteria_id = 0 and candidate_id = '".$j."' ");
                $row = mysql_fetch_array($result);
                $score = $row['score'];
                ?>
        ...
        for($k = 1; $k<= $criteria_num; $k++ ){
            ...
            <tr>
                ...
                for($j = 1; $j <=$candidate_num; $j++){
                    $result = mysql_query("select * from score_backup where decision_id = '".$_GET['decision_id']."' and user_id = '".$i."' and criteria_id = '".$k."' and candidate_id = '".$j."' ");
                    $row = mysql_fetch_array($result);
                    $score = $row['score'];
                    ?>
                    <td><?php echo $score;?></td>
...
<script>
    ...
    function loadXMLDoc()
    {
        ...
        xmlhttp.onreadystatechange=function()
        {
            ...
            xmlhttp.open("GET","overall_data2.php?decision_id=<?php echo $_GET['decision_id']?>",true);
        xmlhttp.send();
